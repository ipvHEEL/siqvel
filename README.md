...
- Поддержка популярных SQL-баз через `SQLAlchemy`:
  ...
  - PostgreSQL
  - MySQL
- Понятный одностраничный интерфейс с возможностями:
  - просмотр уже добавленных данных,
  - создание,
  - редактирование,
  - удаление.
...
3. После внесения данных используйте стандартный скриншот ОС (например, `Win+Shift+S` на Windows, `Cmd+Shift+4` на macOS).

Для автоматизации можно использовать Playwright/Selenium в отдельном e2e-скрипте.
